...
# locust -f listCommunityMembers.py -u 10000 -r 1 -t 1m --headless
...
class MyReqRes(HttpUser):
    ...
    def get(self):
        query="""
            query listCommunityMembers($communityId:String!) {
                listCommunityMembers(communityId: $communityId){ 
                    success 
                    message 
                    communityMemberList{ userId role muted status}} }
        """
        response=self.client.post(
            ...
            json={"query":query,"variables":{"communityId":"61c35cc95e00e228eedfb878"}}
        )
